...
It is important to note that the algorithm goes through all the words from the dictionary that are the same length or +/- 1 length from the misspelled word (O(n)). Thereafter, the algoritm calculates the edit distance between the misspelled word and the words from the dictionary (O(mn)). This combined loop is the point of slowness in the application, as the time complexity increases to O(nmn). Space complexity does not increase similarly, as individual words are compared, and the highest combined space requirement comes from the distance calculation, i.e. O(mn). 
