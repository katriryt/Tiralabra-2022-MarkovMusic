...
User interface is very simple. It consists of one class, [UI](../src/ui/ui.py), that is responsible for interacting with the user, and calls for relevant methods from the [SpellCheck](../src/services/spell_check.py) class.
...
* Class [DistanceHeuristics](../src/services/heuristics.py) provides methods to calculate distance heuristic between two characters in a keyboard. It is used by the [SpellCheck](../src/services/spell_check.py) class in calculation of the distance metric.
...
It is important to note that the algorithm goes through all the words from the dictionary that are the same length or +/- 1 character length from the misspelled word (k stands for the number of these words). Thereafter, the algoritm calculates the edit distance between the misspelled word and the words from the dictionary (O(mn)). This combined loop is the point of slowness in the application, as the time complexity increases to O(kmn). Space complexity does not increase similarly, as individual words are compared, and the highest combined space requirement comes from the distance calculation, i.e. O(mn). 
